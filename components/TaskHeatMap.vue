...
export default defineComponent({
  ...
  methods: {
    async fetchTaskExecutions() {
      const response = await fetch(`/api/taskMapping?taskId=${this.taskId}`);
      if (!response.ok) throw new Error('Network response was not ok');
      const data = await response.json();
      return data;
    },
    async generateYearData() {
      const data = await this.fetchTaskExecutions();
      const yearData = Array.from({ length: 12 }, (_, month) => ({
        ...
        data: Array.from({ length: 31 }, (_, day) => {
          ...
          const executionsForDay = data.filter((task: any) => new Date(task.date).toDateString() === date.toDateString());
          const totalExecutions = executionsForDay.reduce((sum: number, task: any) => sum + Number(task.executions), 0);
          const totalErrors = executionsForDay.reduce((sum: number, task: any) => sum + Number(task.errors), 0);
    ...
    async generateMonthData(monthIndex: number) {
      ...
      const monthData = Array.from({ length: 24 }, (_, hour) => ({
        ...
        data: Array.from({ length: 31 }, (_, day) => {
          ...
          const task = data.find((task: any) => new Date(task.date).toDateString() === date.toDateString() && parseInt(task.hour) === hour);
          return { x: (day+1).toString(), y: task ? (this.selectedDataType === 'executions' ? task.executions : task.errors) : 0, errors: task ? task.errors : 0 };
        })
      }));
      return monthData;
    },
