...
export default defineComponent({
  ...
  methods: {
    async fetchRoutineExecutions() {
      const response = await fetch(`/api/routineMapping?routineId=${this.routineId}`);
    ...
    async generateYearData() {
      ...
      const yearData = Array.from({ length: 12 }, (_, month) => ({
        ...
        data: Array.from({ length: 31 }, (_, day) => {
          ...
          const executionsForDay = data.filter((routine: any) => new Date(routine.date).toDateString() === date.toDateString());
          const totalExecutions = executionsForDay.reduce((sum: number, routine: any) => sum + Number(routine.executions), 0);
          const totalErrors = executionsForDay.reduce((sum: number, routine: any) => sum + Number(routine.errors), 0);
    ...
    async generateMonthData(monthIndex: number) {
      ...
      const monthData = Array.from({ length: 24 }, (_, hour) => ({
        ...
        data: Array.from({ length: 31 }, (_, day) => {
          ...
          return { x: (day+1).toString(), y: routine ? (this.selectedDataType === 'executions' ? routine.executions : routine.errors) : 0, errors: routine ? routine.errors : 0 };
        })
      }));
      return monthData;
    },
